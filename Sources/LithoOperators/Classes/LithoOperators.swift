...
public func tupleMap<T, U>(_ f: @escaping (T) -> Void, _ g: @escaping (U) -> Void) -> (T, U) -> Void {
    return { t, u in
        f(t)
        g(u)
    }
...
public func tupleMap<T, U, V>(_ f: @escaping (T) -> Void, _ g: @escaping (U) -> Void, _ h: @escaping (V) -> Void) -> (T, U, V) -> Void {
    return { t, u, v in
        f(t)
        g(u)
        h(v)
...
public func tupleZip<T, U, V, W, X, Y>(_ f: @escaping (T) -> W, _ g: @escaping (U) -> X, _ h: @escaping (V) -> Y) -> (T, U, V) -> (W, X, Y) {
    return { t, u, v in
        (f(t), g(u), h(v))
    }
}
